...
function cfgValApache(array $config, string $key, string $default = ''): string {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1"><i class="bi bi-sliders me-2"></i>Configuração Global do Apache</h4>
        <small class="text-muted">
            Grava um arquivo próprio (<code>conf-available/rd-intranet.conf</code>) — o <code>apache2.conf</code> da distro nunca é tocado.
            A configuração é validada com <code>apache2ctl configtest</code> antes de ser aplicada; se ficar inválida, é desfeita automaticamente.
        </small>
    </div>
    <a href="<?= url('/apache/dashboard') ?>" class="btn btn-sm btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i>Dashboard Apache
    </a>
</div>

<form method="POST" action="<?= url('/apache/configuracao/salvar') ?>" id="form-config-apache">
            ...
            <div class="col-md-6">
                ...
                <?php elseif ($tipo === 'number'): ?>
                    <input type="number" class="form-control form-control-sm"
                        name="<?= htmlspecialchars($key) ?>" value="<?= $atual ?>">
                <?php else: ?>
...
<div class="cfg-card card">
    <div class="card-header d-flex align-items-center gap-2">
        ...
        <strong>Backups da configuração</strong>
...
(function() {
    ...
    document.getElementById('form-config-apache').addEventListener('submit', function(e) {
        if (!confirm('Confirma a aplicação da nova configuração do Apache?\n\nUm backup será criado automaticamente.')) {
            e.preventDefault();
        }
    });
    ...
    document.addEventListener('click', async function(e) {
        ...
        try {
            ...
            var res = await fetch('<?= url('/apache/configuracao/restaurar') ?>', {method:'POST', body:fd});
...
$titulo = 'Configuração Global do Apache';
