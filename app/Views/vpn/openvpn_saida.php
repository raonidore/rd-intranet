...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="mb-1"><i class="bi bi-box-arrow-up-right me-1"></i> OpenVPN - Conexões de Saída</h4>
        <small class="text-muted">
            <a href="<?= url('/vpn') ?>"><i class="bi bi-arrow-left"></i> Dashboard VPN</a> ·
            Este servidor conectando como <strong>cliente</strong> a um OpenVPN de terceiros (matriz, provedor, outro servidor).
        </small>
    </div>
    <a href="<?= url('/vpn/openvpn/saida/novo') ?>" class="btn btn-primary">
...
(function () {
    const URLS = {
        conectar: <?= json_encode(url('/vpn/openvpn/saida/conectar')) ?>,
        desconectar: <?= json_encode(url('/vpn/openvpn/saida/desconectar')) ?>,
        boot: <?= json_encode(url('/vpn/openvpn/saida/boot')) ?>,
        remover: <?= json_encode(url('/vpn/openvpn/saida/remover')) ?>,
    };
...
$titulo = 'VPN - OpenVPN - Conexões de Saída';
