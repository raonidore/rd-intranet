...
function formatBytesOvpnTrafego(int $bytes): string
{
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="mb-1"><i class="bi bi-bar-chart-line me-1"></i> OpenVPN - Tráfego</h4>
...
<?php else: ?>
    ...
    <form method="get" action="<?= url('/vpn/openvpn/trafego') ?>" class="mb-4">
                ...
                <div class="mapa-calor">
                    <?php foreach ($historico as $h): ?>
                        ...
                        <div class="mapa-celula text-bg-<?= $ativo ? 'success' : 'secondary' ?>"
                             title="<?= htmlspecialchars(data_br($h['coletado_em'])) ?>: <?= formatBytesOvpnTrafego((int)$h['rx_bytes']) ?> ↓ / <?= formatBytesOvpnTrafego((int)$h['tx_bytes']) ?> ↑"
                    ...
                    <tbody>
                        <?php foreach ($historico as $h): ?>
                            <tr>
                                ...
                                <td class="small"><?= $h['conectado_desde'] ? htmlspecialchars(data_br($h['conectado_desde'])) : '—' ?></td>
                                <td class="text-end font-monospace"><?= formatBytesOvpnTrafego((int)$h['rx_bytes']) ?></td>
                                <td class="text-end font-monospace"><?= formatBytesOvpnTrafego((int)$h['tx_bytes']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
(function () {
    ...
    if (botaoAtivarColeta) {
        botaoAtivarColeta.addEventListener('click', async function () {
            ...
            try {
                const res = await fetch(<?= json_encode(url('/vpn/openvpn/ativar-coleta')) ?>, { method: 'POST' });
