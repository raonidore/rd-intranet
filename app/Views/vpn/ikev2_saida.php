...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="mb-1"><i class="bi bi-box-arrow-up-right me-1"></i> IKEv2 - Conexões de Saída</h4>
        <small class="text-muted">
            <a href="<?= url('/vpn') ?>"><i class="bi bi-arrow-left"></i> Dashboard VPN</a> ·
            Este servidor conectando como <strong>iniciador</strong> a um gateway IPsec remoto.
        </small>
    </div>
    <a href="<?= url('/vpn/ikev2/saida/novo') ?>" class="btn btn-primary">
                ...
                <tbody>
                    <?php foreach ($conexoes as $c): ?>
                        <tr>
                            ...
                            <td class="small"><?= htmlspecialchars($c['servidor_remoto']) ?></td>
                            <td><?= $c['tipo_auth'] === 'psk' ? 'PSK' : 'EAP (' . htmlspecialchars($c['usuario_eap']) . ')' ?></td>
...
(function () {
    const URLS = {
        conectar: <?= json_encode(url('/vpn/ikev2/saida/conectar')) ?>,
        desconectar: <?= json_encode(url('/vpn/ikev2/saida/desconectar')) ?>,
        boot: <?= json_encode(url('/vpn/ikev2/saida/boot')) ?>,
        remover: <?= json_encode(url('/vpn/ikev2/saida/remover')) ?>,
    };
...
$titulo = 'VPN - IKEv2 - Conexões de Saída';
