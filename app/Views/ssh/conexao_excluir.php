        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <div class="mb-4">
                    ...
                    <small class="text-muted"><?= htmlspecialchars($conexao['usuario']) ?>@<?= htmlspecialchars($conexao['host']) ?>:<?= (int)$conexao['porta'] ?></small>
                </div>
                ...
                <form method="post" action="<?= url('/ssh/conexoes/excluir') ?>">
                    <input type="hidden" name="id" value="<?= (int)$conexao['id'] ?>">

                    <button type="submit" class="btn btn-danger">
                        <i class="bi bi-trash"></i> Sim, excluir conexão
                    </button>
                    <a href="<?= url('/ssh/conexoes') ?>" class="btn btn-secondary">Cancelar</a>
                </form>
            </div>
        </div>
    </div>
