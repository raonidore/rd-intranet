...
class VpnIkev2Controller extends Controller
{
    private const NOME_JOB_CRON = 'Coleta de tráfego VPN (IKEv2)';

    private VpnIkev2Service $service;

    public function __construct()
    {
        $this->service = new VpnIkev2Service();
    }

    public function servidor(): void
    {
        AuthMiddleware::checkModulo('vpn_ikev2_servidor');

        $iptables = new IptablesService();

        $this->view('vpn/ikev2_servidor', [
    ...
    public function instalar(): void
    {
        ...
        AuditService::registrar('VPN IKEv2', 'Instalar', $resultado['message'] ?? '');
    ...
    public function inicializarPki(): void
    {
        ...
        AuditService::registrar('VPN IKEv2', 'Inicializar PKI', $resultado['message'] ?? '');
    ...
    public function salvarConfig(): void
    {
        ...
        if ($resultado['success']) {
            AuditService::registrar('VPN IKEv2', 'Salvar configuração do servidor', $resultado['message'] ?? '');
        }
        $this->notificarEVoltar($resultado, '/vpn/ikev2/servidor');
    }
    ...
    public function exporToggle(): void
    {
        ...
        AuditService::registrar('VPN IKEv2', $expor ? 'Expor à internet' : 'Deixar de expor à internet', $resultado['message'] ?? '');
    ...
    {
        AuthMiddleware::checkModulo('vpn_ikev2_trafego');
        header('Content-Type: application/json');
        ...
        $resultado = (new CronService())->criar([
            ...
            'descricao' => 'Grava snapshot de tráfego por cliente IKEv2 (VPN > IKEv2 > Tráfego).',
            'expressao' => '*/5 * * * *',
            'usuario_execucao' => 'www-data',
            'comando' => 'php /var/www/rd.intranet/rd vpn:coletar-ikev2',
            'ativo' => true,
        ]);

        AuditService::registrar('VPN IKEv2', 'Ativar coleta de tráfego', $resultado['message']);
    ...
    {
        AuthMiddleware::checkModulo('vpn_ikev2_clientes');

        $this->view('vpn/ikev2_clientes', [
            'status' => $this->service->status(),
        ]);
    }
    ...
    public function criarCliente(): void
    {
        ...
        $resultado = $this->service->criarCliente($nome, $senha);

        if ($resultado['success']) {
            AuditService::registrar('VPN IKEv2', 'Criar cliente', "Cliente \"{$nome}\" criado.");
        }
    ...
    public function revogarCliente(): void
    {
        ...
        AuditService::registrar('VPN IKEv2', 'Revogar cliente', "Cliente #{$id} revogado.");

        echo json_encode($resultado);
    }

    public function baixarCa(): void
    {
        AuthMiddleware::checkModulo('vpn_ikev2_clientes');
        header('Content-Type: application/json');

        $ca = $this->service->caCertificado();

        echo json_encode($ca !== null
            ? ['success' => true, 'ca' => $ca]
            : ['success' => false, 'message' => 'Certificado da CA não disponível.']);
    }
