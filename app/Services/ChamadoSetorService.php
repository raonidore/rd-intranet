...
class ChamadoSetorService
{
    ...
    public function listar(): array
    {
        return $this->pdo->query(
            "SELECT s.*, COUNT(su.usuario_id) AS total_usuarios
             ...
             LEFT JOIN chamados_setor_usuarios su ON su.setor_id = s.id
             GROUP BY s.id
             ORDER BY s.nome"
    ...
    {
        return $this->pdo->query("SELECT * FROM chamados_setores WHERE ativo = 1 ORDER BY nome")->fetchAll(PDO::FETCH_ASSOC);
    }

    public function buscar(int $id): ?array
    {
        $stmt = $this->pdo->prepare('SELECT * FROM chamados_setores WHERE id = ?');
        $stmt->execute([$id]);
    ...
    public function criar(string $nome): array
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO chamados_setores (nome) VALUES (?)');
        $stmt->execute([$nome]);
    ...
    public function atualizar(int $id, string $nome, bool $ativo): array
    {
        ...
        $stmt = $this->pdo->prepare('UPDATE chamados_setores SET nome = ?, ativo = ? WHERE id = ?');
    ...
    {
        $stmt = $this->pdo->prepare('DELETE FROM chamados_setores WHERE id = ?');
        $stmt->execute([$id]);
    ...
    {
        $stmt = $this->pdo->prepare('SELECT setor_id FROM chamados_setor_usuarios WHERE usuario_id = ?');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT usuario_id FROM chamados_setor_usuarios WHERE setor_id = ?');
    ...
    public function salvarUsuariosDoSetor(int $setorId, array $usuarioIds): array
    {
        ...
        try {
            $del = $this->pdo->prepare('DELETE FROM chamados_setor_usuarios WHERE setor_id = ?');
            $del->execute([$setorId]);

            if ($usuarioIds) {
                $ins = $this->pdo->prepare('INSERT INTO chamados_setor_usuarios (setor_id, usuario_id) VALUES (?, ?)');
    ...
    {
        $sql = 'SELECT id FROM chamados_setores WHERE nome = ?';
        $params = [$nome];
